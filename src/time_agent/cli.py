"""Console runner for the time agent."""
...
from .agent import create_time_agent

_EXIT_COMMANDS = {"exit", "quit"}
_DEFAULT_APP_NAME = "time_agent_app"
...
def _ensure_api_key() -> str:
  ...
  if not api_key:
    raise RuntimeError(
        "Set GOOGLE_API_KEY or GENAI_API_KEY before running the time agent."
    )
  return api_key
...
async def run_cli() -> None:
  """Starts the time agent in an interactive console loop."""
  load_dotenv()
  _ensure_api_key()
  ...
  print("Time agent ready. Ask for the current time! Type 'exit' to quit.")
  try:
    while True:
      try:
